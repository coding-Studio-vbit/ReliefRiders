...
router.put("/requester/updateProfile", verifyToken, function(req,res) {
  requester.findOneAndUpdate({phoneNumber: req.user.phoneNumber }, {$set:{name:req.body.name, yearOfBirth: req.body.yearOfBirth, defaultAddress: {addressLine: req.body.addressLine, city: req.body.city, pincode: req.body.pincode}}}, {new: true}, function(err, doc) {
    if (err) {
      res.json({
      status:"failure",
      message: "Unable to retrieve data"
